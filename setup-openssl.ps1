...
Write-Host "`n🔧 Setting variables for current session..." -ForegroundColor Yellow
...
if (Test-Path "$opensslBin\openssl.exe") {
    ...
    try {
        ...
        Write-Warning "   ⚠️  Could not run OpenSSL command"
    }
} else {
...
foreach ($libFile in $libFiles) {
    if (Test-Path $libFile) {
        ...
        Write-Warning "   ⚠️  Missing: $libFile"
        $allLibsFound = $false
    }
}
...
if (Test-Path "$opensslInclude\openssl") {
    ...
    Write-Warning "   ⚠️  Include directory not found"
}
...
if ($allLibsFound) {
    Write-Host "`n✅ OpenSSL setup completed successfully!" -ForegroundColor Green
    Write-Host @"

⚠️  IMPORTANT: You need to restart your terminal or IDE for the environment variables to take effect.
...
"@ -ForegroundColor Yellow
    ...
    Write-Warning "`n⚠️  OpenSSL setup completed with warnings. Some files may be missing."
